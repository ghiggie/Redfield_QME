...
!---------------------------------------------------------------------!
    ...
    namelist/params/N,time_limit,time_interval,trunc,temp,gamma,lambda,rho0,HS,VS,VI
    ...
    do i = 0, N - 1
        ...
        k3 = k2
        ! Calculate k4
        k4 = rk4_int(t+dti,N,VI,temp,lambda,gamma)
        ! Calculate rho(i+1)
    ...
    open(10, file='hermitian.out')
    do i = 0, N
        t_i = i * dti
        tmpl = test_hermitian(rho(i,:,:))
        write(10, '(f10.3,L1)') t_i, tmpl
    end do
    close(10)

    open(10, file='trace.out')
    do i = 0, N
        t_i = i * dti
